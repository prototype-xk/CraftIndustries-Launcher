...
// IIFE : évite le conflit entre `const api` et la globale non-configurable
// `window.api` exposée par contextBridge (sinon SyntaxError, script avorté).
(function () {
...
const sound = (function () {
  ...
  function blip(freq, dur, type, vol) {
    ...
    o.start(); o.stop(c.currentTime + (dur || 0.12) + 0.03);
  }
  function startAmbient() {
    const c = ac(); if (!c || ambient) return;
    const master = c.createGain(); master.gain.value = 0.0001; master.connect(c.destination);
    const lp = c.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = 560; lp.Q.value = 0.6; lp.connect(master);
    const oscs = [110, 164.81, 220].map((f) => {
      const o = c.createOscillator(); o.type = 'sine'; o.frequency.value = f; o.detune.value = Math.random() * 8 - 4; o.connect(lp); o.start(); return o;
    });
    const lfo = c.createOscillator(); lfo.frequency.value = 0.06;
    const lfoG = c.createGain(); lfoG.gain.value = 0.012; lfo.connect(lfoG); lfoG.connect(master.gain); lfo.start();
    master.gain.setTargetAtTime(0.03, c.currentTime, 2.5);
    ambient = { master, oscs, lfo };
  }
  function stopAmbient() {
    if (!ambient) return; const c = ctx; const a = ambient; ambient = null;
    try { a.master.gain.setTargetAtTime(0.0001, c.currentTime, 1); } catch { /* */ }
    setTimeout(() => { try { a.oscs.forEach((o) => o.stop()); a.lfo.stop(); } catch { /* */ } }, 1600);
  }
  return {
...
function setProgress(pct) {
  ...
  el.overlayBar.style.width = v + '%';
  el.overlayPct.textContent = Math.round(v) + '%';
}
...
function fmtDuration(ms) {
  if (!ms || ms < 60000) return ms ? "moins d'1 min" : '—';
...
document.querySelectorAll('.nav__item').forEach((item) => {
  item.addEventListener('click', () => { sound.click(); showView(item.dataset.view); });
});
...
el.btnAccount.addEventListener('click', async () => {
  ...
  if (res.ok) { setAccount(res.profile); setStatus(`Bienvenue ${res.profile.name} !`); sound.success(); }
...
function showBanner(text, isMaint) {
  el.bannerText.textContent = text;
  el.banner.classList.toggle('banner--maint', !!isMaint);
  el.banner.classList.remove('hidden');
  el.content.classList.add('has-banner');
}
async function loadModpackInfo() {
  ...
  if (i.overrides) { el.overridesCard.classList.remove('hidden'); el.overridesSize.textContent = fmtSize(i.overrides.size); }
  else el.overridesCard.classList.add('hidden');
  ...
  if (i.server && i.server.ip) {
    ...
    el.homeServer.textContent = `Adresse : ${i.server.ip}:${i.server.port || 25565}`;
    pingServer();
...
async function loadNews() {
  ...
  for (const n of items) {
    ...
    const tag = n.tag ? `<span class="newscard__tag"></span>` : '';
    card.innerHTML = tag + '<div class="newscard__head"><span class="newscard__title"></span><span class="newscard__date"></span></div><div class="newscard__body"></div>';
...
async function loadScreens() {
  const list = await api.listScreenshots();
  el.screensGrid.innerHTML = '';
  ...
  for (const s of list) {
    ...
    cell.querySelector('.shot__name').textContent = s.name;
    cell.addEventListener('click', () => { el.lightboxImg.src = url; el.lightbox.classList.remove('hidden'); });
...
el.btnPlay.addEventListener('click', async () => {
  ...
  if (!res.ok) {
    setStatus('Erreur : ' + res.error); sound.error(); hideOverlay();
...
el.overlayConsole.addEventListener('click', () => {
  const hidden = el.overlayLog.classList.toggle('hidden');
  el.overlayConsole.textContent = hidden ? 'Afficher la console' : 'Masquer la console';
});
...
api.onCrash((c) => {
  ...
  crashFile = c.file || '';
  el.crashLog.textContent = c.content || 'Aucun rapport disponible.';
...
el.crashCopy.addEventListener('click', async () => {
  try { await navigator.clipboard.writeText(el.crashLog.textContent); el.crashCopy.textContent = 'Copié ✓'; setTimeout(() => { el.crashCopy.textContent = 'Copier'; }, 1500); } catch { /* */ }
});
...
async function loadSettings() {
  ...
  sound.setEnabled(el.setSounds.checked);
  sound.setAmbientWanted(el.setAmbient.checked);
}
...
el.btnSave.addEventListener('click', async () => {
  ...
  await api.saveSettings({
    ramMax: gb * 1024, ramMin: Math.min(2048, gb * 1024),
    javaPath: el.setJava.value.trim(), keepLauncherOpen: el.setKeep.checked,
    directJoin: el.setDirect.checked, uiSounds: el.setSounds.checked, ambientMusic: el.setAmbient.checked
  });
  sound.setEnabled(el.setSounds.checked);
  sound.setAmbientWanted(el.setAmbient.checked);
...
el.btnRepair.addEventListener('click', async () => {
  ...
  el.btnRepair.textContent = 'Forcer la resynchro'; el.btnRepair.disabled = false;
...
api.onProgress((p) => {
  ...
  if (p.phase === 'mods') {
    ...
    if (p.file && p.size) setStatus(`Mod ${p.file} — ${Math.round(frac * 100)}%`);
  } else if (p.phase === 'overrides') {
    const frac = p.size ? p.recv / p.size : 0; pct = frac * 100;
    setStatus(`Configs & scripts — ${Math.round(frac * 100)}%`);
  } else if (p.phase === 'mc') {
    const cur = typeof p.task === 'number' ? p.task : p.current;
    if (typeof cur === 'number' && p.total) pct = (cur / p.total) * 100;
    if (p.type) setStatus(`Minecraft : ${p.type}…`);
  }
  setProgress(pct);
});
api.onLog((line) => {
  ...
  el.overlayLog.textContent += line + '\n';
...
api.onUpdate((u) => {
  const toast = $('update-toast'), text = $('update-text'), btn = $('btn-update');
...
function initParticles() {
  ...
  function build() {
    ...
    particles = Array.from({ length: Math.min(90, Math.max(34, Math.round(w / 15))) }, make);
  }
  let phase = 0;
  function frame() {
    ctx.clearRect(0, 0, w, h); phase += 0.02;
    ctx.shadowColor = 'rgba(41,198,232,0.8)'; ctx.shadowBlur = 6;
    for (const p of particles) {
      ...
      ctx.fillStyle = 'rgba(41,198,232,' + Math.max(0, alpha).toFixed(3) + ')'; ctx.fill();
    }
...
(async function init() {
  ...
  if (res.ok && res.profile) { setAccount(res.profile); setStatus(`Bienvenue ${res.profile.name} !`); }
